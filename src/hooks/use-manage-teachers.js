...
const useManageTeachers = () => {
  ...
  const fetchUsers = async () => {
    ...
    await axios.get(`${axiosUrl}/users/fetch?role=teacher`, axiosConfig)
      .then(({data}) => {
        ...
        var filtered = data.data.filter(data => {
          if (searchBy === 'school_year' || searchBy === 'department') {
  ...
  const exportToCSV = async () => {
    ...
    XLSX.writeFile(workbook, `Teachers-${moment(new Date()).format('YYYY-MM-DD')}.xlsx`);
  }
    ...
    getUser().role === 'ADMIN' && {
      ...
      actionItems: [
        { 
          ...
          label: 'Edit', 
          action: (e, rowData) => navigate('/admin/manage-teachers/edit', { state: rowData }),
        },
        { 
