...
const useSubjectAssignTaskPerformancesForm = ({
  isModalOpen,
  handleClose,
  ...
  const handleUpload = (e) => {
    ...
    reader.onloadend = async function () {
      ...
      const data = {
        file_name: `taskperformance - ${Math.floor(Date.now() / 1000)} - ${file.name}`,
  ...
  const onSubmit = async (data, e) => {    
    ...
    await axios.post(`${axiosUrl}/subject/add_task_performance`, apiData, axiosConfig)
      .then(({data}) => {
        handleSetAssignedTaskPerformances(data.data);
...
export default useSubjectAssignTaskPerformancesForm;