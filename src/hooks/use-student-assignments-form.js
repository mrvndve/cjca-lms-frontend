...
const useStudentAssignmentsForm = ({
  selected,
  toggleModal,
  fetchSubjectDetails,
}) => {
  ...
  const onSubmit = async (data) => {
    ...
    data['assignment_id'] = selected.id;
    ...
    await axios.post(`${axiosUrl}/student/submit_assignment`, apiData, axiosConfig)
      .then(({data}) => {
        socket.emit("sendNotifications2", {
          teacherIds,
          title: 'Assignment',
          message: `${apiData.user} submitted an assignment.`,
...
export default useStudentAssignmentsForm;