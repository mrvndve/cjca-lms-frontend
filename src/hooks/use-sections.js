...
const useSections = () => {
  ...
  const deleteSection = async () => {
    ...
    await axios.post(`${axiosUrl}/section/delete`, formData, axiosConfig)
      .then(({data}) => {
        selected.map((data) => {
  ...
  const exportToCSV = async () => {
    ...
    XLSX.writeFile(workbook, `Sections-${moment(new Date()).format('YYYY-MM-DD')}.xlsx`);
  }
    ...
    },
    {
      ...
      actionItems: [
        { 
          ...
          label: 'Edit', 
          action: (e, rowData) => navigate('/admin/sections/edit', { state: rowData }),
        },
        { 
