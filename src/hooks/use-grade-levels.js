...
const useGradeLevels = () => {
  ...
  const deleteGradeLevels = async () => {
    ...
    await axios.post(`${axiosUrl}/grade_level/delete`, formData, axiosConfig)
      .then(({data}) => {
        selected.map((data) => {
  ...
  const exportToCSV = async () => {
    ...
    XLSX.writeFile(workbook, `SchoolYears-${moment(new Date()).format('YYYY-MM-DD')}.xlsx`);
  }
    ...
    },
    {
      ...
      actionItems: [
        { 
          ...
          label: 'Edit', 
          action: (e, rowData) => navigate('/admin/grade-levels/edit', { state: rowData }),
        },
        { 
